...
import {
  CreateChapter,
  CreateChapterProps,
} from 'components/Popup/CreateChapter/CreateChapter'
import { CreateFeedback } from 'components/Popup/CreateFeedback/CreateFeedback'
import {
  CreateNews,
  CreateNewsProps,
} from 'components/Popup/CreateNews/CreateNews'
...
export const AdminPanel: React.FC = observer((props) => {
  ...
  return (
    <div
      style={{ position: 'relative', zIndex: 2500520, marginTop: 130 }}
      className="admin-panel">
      <div className="container">
        <Button onClick={() => setActive(ActivePopup.NEWS_POPUP)}>
          Создать News
        </Button>
        <Button onClick={() => setActive(ActivePopup.CHAPTERS_POPUP)}>
          Создать Chapter
        </Button>
        <Button onClick={() => setActive(ActivePopup.PARTNERS_POPUP)}>
          Создать Partner
        </Button>
        <Button onClick={() => setActive(ActivePopup.FEEDBACK_POPUP)}>
          Создать Feedback
        </Button>
        <Button onClick={() => setActive(ActivePopup.DELETE_NEWS)}>
          Delete News
        </Button>
        <Button onClick={() => setActive(ActivePopup.DELETE_PARTNER)}>
          Delete Partner
        </Button>
        <Button onClick={() => setActive(ActivePopup.EDIT_PARTNER)}>
          Edit Partner
        </Button>
        <Button onClick={() => setActive(ActivePopup.EDIT_NEWS_IMAGE)}>
          Edit news' image
        </Button>
