    ...
    <header
      ...
      <div className="container">
        <div className="header-inner">
          <NavLink to={PagesRoutes.MAIN} className="header__logo">
            <Logo width={165} />
          </NavLink>

          <a
            href="tel:89218491469"
            className="header__phone-mobile"
            onClick={clickHandler}>
            <strong>8-921-849-14-69</strong>
          </a>

          <nav className={`nav ${menuActive ? 'active' : undefined}`}>
            <NavLink to={PagesRoutes.MAIN} onClick={clickHandler}>
              Главная страница
            </NavLink>
            <Dropdown
              text="Услуги"
            ...
            <a
              ...
              {isOver ? 'СКОРО!' : 'Обучение экспертов'}
            </a>
            <NavLink
              to={PagesRoutes.PARTNERS_AND_FEEDBACK}
              onClick={clickHandler}>
              Партнеры и отзывы
            </NavLink>
            <NavLink to={PagesRoutes.BLOG} onClick={clickHandler}>
              Блог
            </NavLink>
            <a
              href="tel:89218491469"
          ...
          <div
            onClick={clickHandler}
            style={menuActive ? { cursor: 'pointer' } : { cursor: 'pointer' }}
            className={`header__mobile-wrapper`}>
            <object
              data={burgerMenu}
              className={`header__mobile-menu ${
                menuActive ? 'active' : undefined
              }`}
              id="header-mobile-bar"></object>
          </div>
        </div>
      </div>
    </header>
